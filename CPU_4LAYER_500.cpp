...
int main(int argc, const char * argv[]) {
    ...
    int batch_count = 0;
    ofstream file;
    file.open("/Users/jinxuanxu/Projects/563/FinalProject/563FinalProject/563FinalProject/4Layer_500.txt");
    ...
    vector <float> W2 = random_vector(500*500);
    ...
    for (unsigned i = 0; i < 165; ++i) {
        ...
        vector<float> a2 = sigmoid(dot( a1, W2, BATCH_SIZE, 500, 500));
        vector<float> a3 = sigmoid(dot( a2, W3, BATCH_SIZE, 500, 500 ));
        ...
        // dW3 = a1.T * dz3
        vector<float> dW3 = dot(transpose( &a2[0], BATCH_SIZE, 500 ), dz3, 500, BATCH_SIZE, 500);//a1->a2
        // dz2 = dz3 * W3.T * relu'(a1)
        vector<float> dz2 = dot(dz3, transpose( &W3[0], 500, 500 ), BATCH_SIZE, 500, 500) * sigmoid_d(a2);
        
        // dw2 = a1.T * dz2
        vector<float> dW2 = dot(transpose( &a1[0], BATCH_SIZE, 500 ), dz2, 500, BATCH_SIZE, 500);
        // dz1 = dz2 * W2.T * relu'(a1)
        vector<float> dz1 = dot(dz2, transpose( &W2[0], 500, 500 ), BATCH_SIZE, 500, 500) * sigmoid_d(a1);
        
        // dW1 = X.T * dz1
        vector<float> dW1 = dot(transpose( &b_X[0], BATCH_SIZE, 784 ), dz1, 784, BATCH_SIZE, 500);
