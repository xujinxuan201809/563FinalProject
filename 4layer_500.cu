...
__global__ void kFit(const float* X, const int X_w, const int X_h,
                     ...
                     float* l3, const int l3_w, float* l_3_d,
                     ...
                     )
{
        ...
        dSigmoid(dDot(X, W0, l3, X_h, X_w, l3_w), l3, X_h, l3_w);

        dSigmoid(dDot(l3, W3, l1, X_h, l3_w, l1_w), l1, X_h, l1_w);//l3 * W3
        ...
        // dMartixByMatrixElementwise(dDot_m1_m2T(pred_d, W1, l_2_d, X_h, y_w, l2_w), dSigmoid_d(l2, buffer_2, X_h, l2_w), l_2_d, X_h, l2_w);
        
        // dMartixByMatrixElementwise(dDot_m1_m2T(l_2_d, W2, l_1_d, X_h, l2_w, l1_w), dSigmoid_d(l1, buffer_1, X_h, l1_w), l_1_d, X_h, l1_w);
        
        // dMartixByMatrixElementwise(dDot_m1_m2T(l_1_d, W3, l_3_d, X_h, l1_w, l3_w), dSigmoid_d(l3, buffer_3, X_h, l3_w), l_3_d, X_h, l3_w);

        // dDot_m1T_m2(l2, pred_d, W1, X_h, l2_w, y_w);
        
        // dDot_m1T_m2(l1, l_2_d, W2, X_h, l1_w, l2_w);

        // dDot_m1T_m2(l3, l_1_d, W3, X_h, l3_w, l1_w);
        
        // dDot_m1T_m2(X, l_3_d, W0, X_h, X_w, l3_w);
...
//    cout << "hello" << endl;
    ...
    ofstream file;
    file.open("/home/ruixi/Downloads/output_forward_time_4layers_500.txt");
    ...
    ////////////////////////////WEIGHTS_0
    //initialize the W0, the size of W0 = 784 * 256
    ...
    const long signed int W0_size = L3_SIZE * TRAINING_DIM * sizeof(float);
    ...
    //LAYER_2
    const long signed int L2_size = L2_SIZE * BATCH_SIZE * sizeof(float);//64 * 256 layer 2 output size
    ...
    /////////////////////////////////////////////////WEIGHTS_3
    const long signed int W3_size = L3_SIZE * L1_SIZE * sizeof(float);//L1_SIZE = 128, W2_size = 256 * 128
    float *h_W3 = (float*)malloc(W3_size);
    for (int i=0;i< L3_SIZE *L1_SIZE;i++){
        h_W3[i] = 0.1 * (2.0*rand() / RAND_MAX - 1.0);
    }
    float *d_W3;
    cudaMalloc(&d_W3, W3_size);
    cudaMemcpy(d_W3, h_W3, W3_size, cudaMemcpyHostToDevice);

    //LATER_3
    const long signed int L3_size = L3_SIZE * BATCH_SIZE * sizeof(float);//256 * 256 layer 2 output size

    float* h_layer_3 = (float*)malloc(L3_size);
    float* h_layer_3_delta = (float*)malloc(L3_size);
    float* h_buffer_3 = (float*)malloc(L3_size);

    for (int i = 0; i < L3_SIZE*BATCH_SIZE; i++) {//initialize
        h_layer_3[i] = 0.0;
        h_buffer_3[i] = 0.0;
        h_layer_3_delta[i] = 0.0;
    }

    float *d_layer_3;
    cudaMalloc(&d_layer_3, L3_size);
    cudaMemcpy(d_layer_3, h_layer_3, L3_size, cudaMemcpyHostToDevice);
    float *d_buffer_3;
    cudaMalloc(&d_buffer_3, L3_size);
    cudaMemcpy(d_buffer_3, h_buffer_3, L3_size, cudaMemcpyHostToDevice);
    float *d_layer_3_delta;
    cudaMalloc(&d_layer_3_delta, L3_size);
    cudaMemcpy(d_layer_3_delta, h_layer_3_delta, L3_size, cudaMemcpyHostToDevice);
    ///////////////////////////////////////////////////////////////////
...
batch_start = clock();
        ...
        kFit << < 1, 1 >> > (d_X, TRAINING_DIM, BATCH_SIZE,
                             ...
                             d_layer_3, L3_SIZE, d_layer_3_delta,
